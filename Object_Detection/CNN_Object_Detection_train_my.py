...
class WordDataset(utils.Dataset):

    def load_data(self, data_dir):
        self.add_class("shapes", 1, "a")
        list_of_files = {}
    ...
    def load_image(self, image_id):
        ...
        img = cv2.imread(info['path']+'.png')
        # print(img.shape, info['path'])
        return img
    ...
    def load_mask(self, image_id):
        ...
        """
        info = self.image_info[image_id]
        with open(info['path']+'.p', 'rb') as fp:
            mask = pickle.load(fp)

        sz = mask.shape
        if len(sz)==2:
            mask = mask.reshape(sz[0], sz[1], 1)
        channel = mask.shape[2]
        class_ids = np.ones(channel)
...
if __name__ == '__main__':
    ...
    dataset_train.set_data(data_train)

...
def get_poses(images):
    ...
    for image_id in images:
        ...
        A = Utils.get_A(rot_mat_inv)
        img_dic[images[image_id].name] = Utils.create_Q(A, cen)
    return img_dic
...
if __name__ == '__main__':
    ...
    cameras, images, points3D = read_model(path='{}/sparse/0/'.format(wd), ext='.bin')
    img_dic = get_poses(images)
    ...
    for seq in poses_dic:
        ...
        for id in poses:
            if pre is not None:
                p1 = pre
                p2 = poses[id]
                A, T = Utils.get_relative_A_T(p1.Q4, p2.Q4)

                q1 = img_dic[os.path.basename(p1.filename)]
        ...
        # for id in poses:
        #     pose = poses[id]
        #     filename = os.path.basename(pose.filename)
        #     Q0 = img_dic[filename]
        #     Q2 = pose.Q4
        #     A ,T = Utils.get_relative_A_T(Q0, Q2)
        #
        #     print filename, A[0], A[1], A[2], T[0], T[1], T[2]
        ...
        for a in range(iterations):
            ...
            for lp in range(cfg.loop):
                ...
                for c in range(0, length, sz):
                    dd = data[c:c +sz]
                    th = truth[c:c + sz]
                    feed = {input_dic['data_1']: np.array(dd), input_dic['data_2']: np.array(dd)}
                ...
                for c in range(0, length, cfg.batch_size):
                    ...
                    dd = np.array(dd)
                    feed = {input_dic['data_1']: dd,input_dic['data_2']: dd, outputs[0]: th1, outputs[1]: th2 }
                    A, _ = sess.run([losses, opts], feed_dict=feed)
                    # te_loss += A
                    diff_loss1 += A[0] #np.sum((B[:, :2]-th[:, :2])*(B[:, :2]-th[:,:2]))
                    diff_loss2 += A[1] # np.sum((B[:, 2]-th[:, 2])*(B[:, 2]-th[:, 2]))
                    te_count += len(th)

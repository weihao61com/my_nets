...
def run_data(rst_dic, truth_dic, fname):
    ...
    for id in rst_dic:
        ...
        for a in range(len(t)):
            fp.write('{},{},{},'.format(t[a], mm[a], mm1[a]))
...
class rNet(Network):
    ...
    def parameters(self, cfg):
        ...
        ws.append(self.create_ws('out', ins, cfg.Nout))
        self.ws.append(ws)
...
def run_test(input_dic, sess, xy, te, cfg, mul=1):
    ...
    truth_dic = {}
    for a in range(mul):
        tr_pre_data = te.prepare(rd=False, multi=-1)
...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        ...
        for a in range(iterations):
            ...
            att = cfg.att
            for _ in range(loop):
                tr_pre_data = tr.prepare_evt(multi=cfg.batch_size)

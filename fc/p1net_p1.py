...
class P1Net:
    ...
    def train(self, data):
        ...
        _, ref, loss2 = sess.run([self.opt2, self.reference2, self.loss2], feed_dict=feed)
    ...
    def run(self, data, sess):
        ...
        for d in data:
            ...
            #it.append(ts)
        ref = reference
        input_data = np.concatenate((ref, np.array(id)), axis=1)
        feed = {self.input2: input_data}
        ref, result2 = sess.run([self.reference2, self.xy2], feed_dict=feed)
